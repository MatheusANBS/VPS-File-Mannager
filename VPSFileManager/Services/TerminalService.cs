...
namespace VPSFileManager.Services
{
    public class TerminalService : IDisposable
    {
        ...
        // Decoder com estado para lidar com sequências UTF-8 multi-byte entre leituras
        private Decoder _utf8Decoder = Encoding.UTF8.GetDecoder();
        ...
        public async Task ConnectAsync(Models.ConnectionInfo connectionInfo, int columns, int rows, bool rawMode = true)
        {
            ...
            await Task.Run(() =>
            {
                ...
                // Resetar decoder UTF-8 para nova conexão
                ...
                while (!token.IsCancellationRequested && _shellStream != null)
                {
                    try
                    {
                        if (_shellStream.DataAvailable)
                        {
                            ...
                            if (bytesRead > 0)
                            {
                                // Usar Decoder com estado para lidar com sequências UTF-8
                                // multi-byte que ficam divididas entre leituras do buffer.
                                // Isso é crítico para apps TUI como glances/htop que usam
                                // muitos caracteres Unicode (box-drawing, blocos, etc.).
                                var charCount = _utf8Decoder.GetChars(buffer, 0, bytesRead, charBuffer, 0);
        ...
        public void Resize(int columns, int rows)
        {
            ...
            // Fallback: usar stty silenciosamente (escondendo output com redireção e sequência de controle)
            try
            {
                _shellStream.Write($"stty cols {columns} rows {rows} 2>/dev/null\n");
            }
            catch { }
        }
